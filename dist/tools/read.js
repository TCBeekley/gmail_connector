...
async function fanOut(account, fn) {
    ...
    const settled = await Promise.allSettled(targets.map(async (a) => ({ alias: a.alias, email: a.email, value: await fn(a.alias) })));
...
export function registerReadTools(server) {
    server.registerTool("search_emails", {
        description: "Search for messages using Gmail's search syntax (e.g. 'from:foo subject:bar after:2025/01/01 has:attachment'). Returns metadata only — call get_email for the full body.",
        inputSchema: {
    ...
    server.registerTool("get_email", {
        description: "Fetch a single message by ID, including headers and parsed text/HTML body. Returns attachment metadata; use get_attachment to download the bytes.",
        inputSchema: {
            account: z.string().describe(ACCOUNT_DESC),
    ...
    server.registerTool("get_thread", {
        description: "Fetch all messages in a thread by thread ID. Returns parsed messages in order.",
        inputSchema: {
            account: z.string().describe(ACCOUNT_DESC),
