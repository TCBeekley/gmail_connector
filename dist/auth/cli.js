...
import { getAccount, listAccounts, putAccount, removeAccount, renameAccount, } from "./token-store.js";
import { resolveMode, scopesFor } from "../config.js";
...
async function waitForCode(server) {
    return new Promise((resolve, reject) => {
...
export async function authCommand(alias) {
    if (!alias) {
    ...
    if (!info.email)
        throw new Error("could not resolve account email from userinfo");
    const account = {
        alias,
        email: info.email,
        scopes,
