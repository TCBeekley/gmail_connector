...
import { ACCOUNTS_PATH, CONFIG_DIR, KEY_PATH } from "../config.js";
...
async function loadKey() {
    ...
    try {
        ...
        if (buf.length !== 32)
            throw new Error(`corrupt key at ${KEY_PATH}`);
        return buf;
    }
    catch (e) {
        if (e.code !== "ENOENT")
...
async function saveAll(state) {
    ...
    const stored = { version: 1, data: encrypt(key, JSON.stringify(state)) };
...
export async function getAccount(aliasOrEmail) {
    const { accounts } = await loadAll();
    if (accounts[aliasOrEmail])
        return accounts[aliasOrEmail];
    for (const acc of Object.values(accounts)) {
        if (acc.email.toLowerCase() === aliasOrEmail.toLowerCase())
            return acc;
    }
    return null;
}
...
export async function updateTokens(alias, tokens) {
    const state = await loadAll();
