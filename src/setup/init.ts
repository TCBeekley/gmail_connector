...
async function findDownloadedJson(): Promise<string | null> {
  ...
  try {
    ...
    const stats = await Promise.all(
      matches.map(async (n) => ({ n, mtime: (await stat(join(dir, n))).mtimeMs })),
    );
...
async function validateKeysFile(path: string): Promise<Validation> {
  let raw: string;
  try {
    raw = await readFile(path, "utf8");
  } catch (e) {
    return { ok: false, reason: `cannot read ${path}: ${(e as Error).message}` };
  }
